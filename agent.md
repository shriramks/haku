...
- **After completing any unit of work**, append an entry to `progress.md`: date heading, one-line title, files changed, 2–4 bullets on what changed and why. Keep it terse. When all sessions of a multi-session task are done, move the entire task block from `## Todo` to `## Done` in `progress.md`.
