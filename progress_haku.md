...
### 36 — Tax Report — Session 2 — `/tax` page + Summary tab
Files: `app/tax/page.tsx`, `app/tax/TaxClient.tsx`, `components/UserMenu.tsx`, `middleware.ts`
- Route shell with FY picker and 3-segment control (Summary | Gains | Harvesting)
- Summary tab: LTCG / STCG / Dividend Income totals, est. tax per type, per-asset-type breakdown
- UserMenu entry point + middleware wiring

### 37 — Tax Report — Session 3 — Gains tab
Files: `app/tax/TaxClient.tsx`
- Per-sell list grouped by asset type with LTCG/STCG badge per row
- Tap → BottomSheet showing consumed buy lots, purchase dates, holding period

### 38 — Tax Report — Session 4 — Harvesting advisory tab
Files: `app/tax/TaxClient.tsx`
- LTCG room: how much more can be booked tax-free this FY, per-stock breakdown
- Loss harvesting: unrealised losses to offset realised STCG gains
- Almost-LTCG: holdings crossing 1-year mark soon with days-remaining countdown
