            ...
            <Link key={row.symbol} href={`/stocks/${row.symbol}`}
              ...
              <div className="px-4 py-3">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-white/40 text-sm tabnum">{formatPct(row.allocationPct)}</span>
                </div>
                ...
                <div className="grid grid-cols-4 gap-1">
                  <SmallMetric label="Budget"  value={formatINR(row.budget)} />
                  <SmallMetric label="Spent"   value={formatINR(row.spent)} />
                  <SmallMetric label="Left"    value={formatINR(row.remaining)}
                               color={row.remaining < 0 ? 'text-red-400' : 'text-white/70'} />
                  <SmallMetric label="% left"  value={formatPct(row.pctRemaining)}
                               color={
                                 row.pctRemaining > 30 ? 'text-green-400' :
                ...
                {/* Holdings row */}
                {row.qty > 0 && (
                  <div className="grid grid-cols-4 gap-1 mt-1.5 pt-1.5 border-t border-white/5">
