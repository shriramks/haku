export default function Loading() {
  return (
    <div className="pb-4">
      ...
      <BottomNavShell active="/allocation" />
    </div>
  )
}
...
function BottomNavShell({ active }: { active: string }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 flex items-center gap-2.5 px-3"
      ...
      <div className="flex items-center justify-around flex-1 p-1.5" style={pill}>
        {tabs.map(({ href, label, icon }) => {
          const isActive = active === href
          return (
            <div key={href} className="flex flex-col items-center gap-0.5 px-2.5 py-1.5 rounded-2xl"
