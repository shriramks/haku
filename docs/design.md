...
- **Status lives on the control, not in a caption.** Saved-price freshness on Portfolio is an amber dot on the Prices button — silent when fresh, `Updating…` / `Updated` / `Retry` as the tap plays out. A holding whose own price is older than the rest's shows its date in the row's 1D slot (same tag style, no colour). A raw timestamp would mislead — yesterday's close is perfectly fresh at 9 am — so the rule ("fetched before the last market close") is encoded in the dot, not printed.
