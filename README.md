...
2. **Buy bands** — live financials fetched from Screener.in and NSE; valuation zones computed per stock by category
...
### Band generation (no AI key needed)
Financials are fetched deterministically — no AI involved:
- **Stocks**: EPS, PAT, ROCE, and Mcap parsed from [Screener.in](https://www.screener.in) consolidated annual P&L
- **Index ETFs**: index level and trailing PE fetched from the NSE API
- **Auto-tranches**: up to 5 buy tranches per stock, distributed within the Buy zone (skewed toward lower prices), qty sized from remaining FY budget

### Investability scoring (Gemini)
The investability scorecard uses **Gemini 2.5 Flash** to score a stock across 10 qualitative gates (moat, management, balance sheet, etc.). To use it, add your own Gemini API key in **Settings** (tap the profile icon). Free keys available at [aistudio.google.com](https://aistudio.google.com).
...
| [docs/app-spec.md](docs/app-spec.md) | App model — vocabulary, screens, investment math |
| [docs/valuation-playbook.md](docs/valuation-playbook.md) | Valuation rules — band multiples, signals, tranches |
